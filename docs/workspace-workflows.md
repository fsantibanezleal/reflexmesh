...
6. `GET /api/workspace/recovery` inspects durable unknown intents while the worker is idle. Each record includes its original resource leases, prior evidence and `record_sha256` binding. A process is eligible for browser recovery only when its receipt proves the direct child stopped and its immutable argv still matches the host registration. Crashes without that evidence require an external verifier using `Runtime.reconcile`.
7. `POST /api/workspace/reconcile` accepts `intent_id`, `record_sha256`, a `resolution` of `completed_verified` or `failed_verified`, and nonempty `checks` containing `{path,sha256}` or `{path,absent:true}`. All leased files must be covered. The operator must supply `reviewed_effects:true`, `processes_quiescent:true`, and a review `note` of 10–2000 characters. The server checks actual file fingerprints before releasing leases, records both the evidence and explicit operator attestation, and invalidates outstanding previews. Missing review, stale bindings and mismatched evidence reject without release. The operator is responsible for effects outside the listed files and for any descendants not covered by OS containment; those claims are not inferred from a file digest. Recovery does not reverse changes.
